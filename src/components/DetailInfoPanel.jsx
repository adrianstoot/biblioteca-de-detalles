  ...
  return (
    <aside className="fixed lg:static top-0 right-0 bottom-0 w-80 sm:w-84 bg-white border-l border-slate-200 flex flex-col z-30 shadow-xl lg:shadow-none select-none">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar text-xs">
        ...
        <div>
          <h4 className="text-[11px] font-semibold text-slate-700 uppercase tracking-wider mb-2">
            {detail.categoryKey === 'tripo_models' ? 'Modelo Escaneado con Textura PBR' : 'Textura y Acabado de Acero'}
          </h4>

          {detail.categoryKey === 'tripo_models' ? (
            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 space-y-2 text-[11px]">
              <div className="flex items-center text-emerald-700 font-medium">
                ...
                <span>Textura PBR original escaneada en alta definición</span>
              </div>
              <div className="pt-1.5 border-t border-slate-200/60 flex flex-wrap gap-1">
                <span className="px-2 py-0.5 bg-white border border-slate-200 rounded-md text-[10px] text-slate-700 shadow-xs">
                  Ferralla de acero B500S
                </span>
                <span className="px-2 py-0.5 bg-white border border-slate-200 rounded-md text-[10px] text-slate-700 shadow-xs">
                  Encofrado de madera y puntales
                </span>
                <span className="px-2 py-0.5 bg-white border border-slate-200 rounded-md text-[10px] text-slate-700 shadow-xs">
                  Hormigón armado
                </span>
            ...
            <div className="space-y-2.5">
              ...
              {detail.categoryKey === 'architectural_models' && (
                <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 space-y-1.5 text-[11px]">
                  <div className="flex items-center text-blue-700 font-medium text-[10px] uppercase tracking-wider">
                    <CheckCircle2 className="w-3 h-3 mr-1 text-blue-600 shrink-0" />
                    <span>Materiales PBR Fotorealistas</span>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    <span className="px-1.5 py-0.5 bg-white border border-slate-200 rounded text-[10px] text-slate-700">
                      Hormigón (Normal + Bump)
                    </span>
                    <span className="px-1.5 py-0.5 bg-white border border-slate-200 rounded text-[10px] text-slate-700">
                      Tornillería galvanizada PBR
                    </span>
                    <span className="px-1.5 py-0.5 bg-white border border-slate-200 rounded text-[10px] text-slate-700">
                      Acero laminado S275
                    </span>
